...
from app.processing.validators import validate_csv
from app.processing.language_mapper import is_valid_language
from typing import Dict, Any, Optional
import os
import logging

logger = logging.getLogger("app.api.routes.csv")
...
@router.post(
    ...
    summary="Translate a CSV file dataset",
    description="Translates cell values of string columns in a CSV, returning a public download URL."
)
async def translate_csv(
    file: UploadFile = File(..., description="CSV file dataset asset"),
    ...
    try:
        ...
        # 2. Call CSVService
        logger.info(f"Processing CSV translation task for file '{file.filename}'...")
        response = await service.translate_csv(
            user_id=user_id,
        ...
        logger.error(f"CSV translation request failed: {str(exc)}", exc_info=True)
        ...
        raise HTTPException(
            ...
            detail=f"CSV translation failed: {str(exc)}"
        )
