...
from app.processing.validators import validate_docx
from app.processing.language_mapper import is_valid_language
from typing import Dict, Any, Optional
import os
import logging

logger = logging.getLogger("app.api.routes.docx")
...
@router.post(
    ...
    summary="Translate a Word DOCX file",
    description="Translates Word document body and table paragraphs, returning a public download URL."
)
async def translate_docx(
    file: UploadFile = File(..., description="DOCX file asset"),
    ...
    try:
        ...
        # 2. Call DocxService
        logger.info(f"Processing DOCX translation task for file '{file.filename}'...")
        response = await service.translate_docx(
            user_id=user_id,
        ...
        logger.error(f"DOCX translation request failed: {str(exc)}", exc_info=True)
        ...
        raise HTTPException(
            ...
            detail=f"Word document translation failed: {str(exc)}"
        )
