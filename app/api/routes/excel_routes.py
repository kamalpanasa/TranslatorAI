...
from app.processing.validators import validate_excel
from app.processing.language_mapper import is_valid_language
from typing import Dict, Any, Optional
import os
import logging

logger = logging.getLogger("app.api.routes.excel")
...
@router.post(
    ...
    summary="Translate an Excel spreadsheet",
    description="Translates Excel sheet cells (excluding formulas) and returns a public download URL."
)
async def translate_excel(
    file: UploadFile = File(..., description="Excel file asset (xlsx)"),
    ...
    try:
        ...
        # 2. Call ExcelService
        logger.info(f"Processing Excel translation task for file '{file.filename}'...")
        response = await service.translate_excel(
            user_id=user_id,
            temp_xlsx_path=temp_path,
        ...
        logger.error(f"Excel translation request failed: {str(exc)}", exc_info=True)
        ...
        raise HTTPException(
            ...
            detail=f"Excel spreadsheet translation failed: {str(exc)}"
        )
